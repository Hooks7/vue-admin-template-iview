    ...
    <aside :class="asideClassName">
      ...
      >
        ...
        <div v-for="(item, index) in menuItems" :key="index">
          <Submenu v-if="item.children" :class="isShowAsideTitle ? '' : 'shrink'" :name="index">
            <template slot="title">
              <Icon :size="item.size" :type="item.type" />
              <span v-show="isShowAsideTitle">{{ item.text }}</span>
            </template>
            <div v-for="(subItem, i) in item.children" :key="index + i">
              <Submenu v-if="subItem.children" :class="isShowAsideTitle ? '' : 'shrink'" :name="index + '-' + i">
                <template slot="title">
                  <Icon :size="subItem.size" :type="subItem.type" />
                  <span v-show="isShowAsideTitle">{{ subItem.text }}</span>
                </template>
                <template v-for="(threeItem, k) in subItem.children">
                  <a v-if="threeItem.isExternal" :key="index + i + k" href="https://www.baidu.com" target="_blank">
                    <MenuItem
              ...
              <template v-else-if="!subItem.hidden">
                <a v-if="subItem.isExternal" :href="subItem.url" target="_blank" class="external">
          ...
          <template v-else-if="!item.hidden">
            <a v-if="item.isExternal" :href="item.url" target="_blank" class="external">
        ...
        <div class="div-tags">
          <ul class="ul-c">
            ...
            >
              ...
              <Icon v-if="item.name != 'home'" size="16" type="md-close" @click="closeTag(index)" />
            </li>
          </ul>
          <!-- 标签栏相关功能 -->
          <div class="div-icons">
...
export default {
  ...
  watch: {
    ...
    crumbs(val) {
      window.localStorage.setItem('crumb-title', val)
    }
  },
  ...
  mounted() {
    ...
    this.crumbs = localStorage.getItem('crumb-title') || '主页'
  ...
  methods: {
    ...
    shrinkAside() {
      ...
      this.openMenus = []
      this.$nextTick(() => {
        if (this.$refs.asideMenu) this.$refs.asideMenu.updateOpened()
      })
